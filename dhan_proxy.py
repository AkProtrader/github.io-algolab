...
def init_tradehull(client_code, api_key, api_secret):
    ...
    try:
        ...
        # Debug: dump all non-method attributes so we can find where token is stored
        print("  Tradehull v3 instance attributes:")
        for attr in sorted(dir(tsl_client)):
        ...
        for attr in ['access_token','token','dhan_token','accessToken',
            ...
            if val and isinstance(val, str) and len(val) > 10:
                dhan_token = val
                print(f"  ✔ Token found at tsl_client.{attr} = {val[:30]}...")
                break

        if dhan_token:
            print(f"  ✔ Tradehull v3 connected! Client: {dhan_client}")
        else:
            print(f"  ⚠ Tradehull v3 connected (client: {dhan_client}) — token attr not found yet")
            print(f"    (share the .attr list above and we will fix it)")
