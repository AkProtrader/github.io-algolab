...
def init_tradehull(client_code, api_key, api_secret):
    ...
    Tradehull v3 API: Tradehull(client_code, api_key, api_secret)
    ...
    try:
        ...
        # Tradehull v3: positional args — client_code, api_key, api_secret
        tsl_client = Tradehull(client_code, api_key, api_secret)
        dhan_client = str(client_code)

        # Debug: print ALL attributes to find where v3 stores the token
        all_attrs = {a: str(getattr(tsl_client, a, ''))[:80]
                     for a in dir(tsl_client) if not a.startswith('__')}
        print("  Tradehull v3 attributes:")
        for k, v in all_attrs.items():
            if v and v != "<class 'method'>" and 'method' not in v.lower() and 'bound' not in v:
                print(f"    {k} = {v}")

        # Try every common token attribute name
        dhan_token = None
        for attr in ['access_token','token','dhan_token','accessToken',
                     'auth_token','bearer_token','jwt','api_token']:
            val = getattr(tsl_client, attr, None)
            if val and isinstance(val, str) and len(val) > 10:
                dhan_token = val
                print(f"  ✔ Token found in tsl_client.{attr}")
                break

        if dhan_token:
            print(f"  ✔ Tradehull connected! Client: {dhan_client}, Token: {dhan_token[:20]}...")
        else:
            print(f"  ✔ Tradehull connected (client: {dhan_client}) — token not found in attrs, using tsl_client directly")
...
def get_dhan_headers():
    """Headers for raw Dhan REST calls."""
    return {
...
def dhan_headers_from_request(req):
    """
    Build headers — prefer proxy-held token (from Tradehull login),
    fall back to whatever the browser sent.
    """
    t = dhan_token  or req.headers.get("access-token", "") or ""
    c = dhan_client or req.headers.get("client-id",    "") or ""
    return {"Content-Type": "application/json", "access-token": t, "client-id": c}

def dhan_get(path, params=None):
    """Make a Dhan API GET — use tsl_client if token unavailable."""
    if tsl_client and not dhan_token:
        # Tradehull v3 may have direct methods; fall through to REST with its session
        try:
            sess = getattr(tsl_client, 'session', None) or getattr(tsl_client, '_session', None)
            if sess:
                return sess.get(f"{DHAN_BASE}{path}", params=params)
        except:
            pass
    return requests.get(f"{DHAN_BASE}{path}", headers=get_dhan_headers(), params=params)
