...
def extract_token(tsl):
    """
    Extract the Dhan access token (JWT) from a Tradehull v3 instance.
    dhanhq stores it as self.access_token inside the .Dhan object.
    """
    obj = getattr(tsl, 'Dhan', None)
    if obj is None:
        return None

    # Direct attribute access — most reliable
    tok = getattr(obj, 'access_token', None)
    ...
    # Check session headers (dhanhq bakes token into session)
    sess = getattr(obj, 'session', None)
    if sess:
        hdrs = getattr(sess, 'headers', {})
        for k in ['access-token', 'Access-Token', 'authorization', 'Authorization']:
            v = hdrs.get(k, '')
            if v and len(v) > 20:
                return v.replace('Bearer ', '')

    # Check header dict attribute
    hdr_dict = getattr(obj, 'header', None) or getattr(obj, 'headers', None)
    if isinstance(hdr_dict, dict):
        for k in ['access-token', 'Access-Token']:
            v = hdr_dict.get(k, '')
            if v and len(v) > 20:
                return v
...
def dhan_rest(method, path, body=None, params=None):
    """Make a raw Dhan REST call using current token."""
