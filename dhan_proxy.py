...
def init_tradehull(client_code, api_key, api_secret):
    ...
    try:
        ...
        # Tradehull v3: token lives inside .Dhan (dhanhq object)
        # tsl_client.Dhan.access_token
        dhan_token = None
        try:
            dhan_obj = getattr(tsl_client, 'Dhan', None)
            if dhan_obj:
                for attr in ['access_token','token','accessToken','auth_token']:
                    val = getattr(dhan_obj, attr, None)
                    if val and isinstance(val, str) and len(val) > 10:
                        dhan_token = val
                        print(f"  ✔ Token found at tsl_client.Dhan.{attr}")
                        break
        except Exception as te:
            print(f"  ⚠ Could not extract token from .Dhan: {te}")

        # Also try top-level attributes as fallback
        if not dhan_token:
            for attr in ['access_token','token','accessToken','auth_token','token_id']:
                val = getattr(tsl_client, attr, None)
                if val and isinstance(val, str) and len(val) > 10:
                    dhan_token = val
                    print(f"  ✔ Token found at tsl_client.{attr}")
                    break

        if dhan_token:
            print(f"  ✔ Tradehull v3 connected! Token: {dhan_token[:20]}...")
        else:
            print(f"  ⚠ Token not extracted — Dhan REST calls will need manual token")
