...
def connect():
    ...
    if test.status_code == 401:
        return jsonify({"success": False,
                        "error": "Token rejected by Dhan — get a fresh token from dhanhq.co → My Profile → Data APIs",
                        "dhan_response": test.json()}), 401
    ...
    # Also init Tradehull with real token (for historical data etc)
    init_tradehull_with_token(cid, tok)

    fund_data = test.json() if test.status_code == 200 else {}
    return jsonify({
        "success":      True,
        "access_token": tok,
        "client_id":    cid,
